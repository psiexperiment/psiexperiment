...
A transient `PermissionError` on a chunk write (antivirus/search indexer/sync client briefly locking the file) used to be retried by wrapping `zarr.Array.append`, which commits the resize *before* writing chunks — so the retry grew the array a second time, leaving a block of fill values and shifting every later sample by one acquisition block (~0.125 s) for the rest of the recording. `_retry_write` now retries the resize and the slice write separately (each is idempotent on its own); never retry `append` as a unit. `_check_continuity` (`base_store.enaml`) logs `NIDAQ_DATA_GAP` when a chunk's `s0` disagrees with what's on disk, which is how such a shift is detected after the fact. **Recordings damaged by this live on** — finding and repairing them is `psidata.repair` (`psidata-scan-recording`, `psidata-repair-recording`), not here: it's post-processing, and the people repairing data have psidata without a psiexperiment checkout.
